'''
A mirror of CouchUtil but save records to
remotely hosted CouchDB

Warning: this file explicitly contains username and password
to publicly accesible CouchDB and if exposed,
can cause potential security problem.
'''
...
def buildCore2(row):
    ...
    if isinstance(row[PARTICIPANT], list):
        ...
        rec['participant']= {'type':'device', 'name':row[PARTICIPANT]}
    if (len(row[COMMENT]) == 0) and (len(row[REASON]) == 0):
        rec['status'] = {'status':'Complete', 'status_qualifier': row[STATUS]}
    elif (len(row[COMMENT]) == 0):        
        rec['status'] = {'status':'Complete', 'status_qualifier': row[STATUS], 'status_qualifier_reason':row[REASON]}
    elif (len(row[REASON]) == 0):
        rec['status'] = {'status':'Complete', 'status_qualifier': row[STATUS], 'comment':row[COMMENT]}
    else:
        rec['status'] = {'status':'Complete', 'status_qualifier': row[STATUS], 'comment':row[COMMENT], 'status_qualifier_reason':row[REASON]}
    return rec
...
def saveList2(doc):
    ...
    proc = {'Environment_Observation':processEnv2, 'State_Change':processState2, 'Agronomic_Activity':processAgro2, 'Phenotype_Observation':processPheno2}    
...
def test():
    print ("Env Rec")
    rec = ['Environment_Observation','', 'Left_Side', 'Air', 'Temperature', 27.5, 'fairenheight', 'SI7021', 'Success', 'Hello', 'Hello']
    saveList2(rec)
