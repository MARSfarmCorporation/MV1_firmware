...
def test_pump(amount):
    ...
    if os.path.exists(pump_lock_file):
        ...
        if time_module.time() - timestamp < pump_lock_timeout:
            # Log skipping the task due to valid lock file
            with open(pump_log_file, 'a') as file:
                file.write(current_time + " - Lock file exists and is valid. Skipping user-triggered task.\n")
            print("Lock file exists and is valid. Skipping user-triggered task.")
            return
        else:
            # Log ignoring the stale lock file
            with open(pump_log_file, 'a') as file:
                file.write(current_time + " - Lock file is stale. Continuing with user-triggered task.\n")
            print("Lock file is stale. Continuing with user-triggered task.")
